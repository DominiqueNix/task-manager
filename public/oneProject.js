...
let completedTaskprogressbarText = document.getElementById(
  "project-task-c-text"
);
...
completedTaskprogressbar.setAttribute(
  "style",
  `width: ${(comp / total || 0) * 100}%`
);
completedTaskprogressbarText.textContent = `${Math.round(
  (comp / total || 0) * 100
)}% of tasks completed`;
...
let currId = document
  .getElementById("one-project-title")
  .getAttribute("data-value");
projectDeletebtn.addEventListener("click", () => {
  handleProjectDelete(currId);
});
...
//default collabs
let currProjectCollabsHTML = document.getElementById(
  "update-chosen-collaborators"
);
let currProjectCollabs = JSON.parse(
  currProjectCollabsHTML.getAttribute("data-value")
);
let currProjectUser = JSON.parse(
  currProjectCollabsHTML.getAttribute("data-user")
);
...
projectUpdateCollaborators.forEach((col) => {
  ...
  closeBtn.setAttribute(
    "onClick",
    "removeUpdateProjectCollaborator(this.parentElement)"
  );
...
//update project
document
  .getElementById("update-project-submit")
  .addEventListener("click", async () => {
    let title = document.getElementById("update-project-title").value.trim();
    let description = document
      .getElementById("update-project-description")
      .value.trim();
...
for (let i = 0; i < eachCheck.length; i++) {
  eachCheck[i].addEventListener("change", () => {
    if (
      eachCheck[i].checked &&
      !Assigneescheckedboxes.includes(eachCheck[i].value)
    ) {
...
document
  .getElementById("add-assignees-btn")
  .addEventListener("click", async (e) => {
    ...
    if (Assigneescheckedboxes && addAssigneeTaskId) {
      ...
      const res = await fetch(
        `/projects/${currId}/tasks/${addAssigneeTaskId}`,
        {
          method: "PUT",
