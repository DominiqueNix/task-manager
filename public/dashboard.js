...
if (taskCheckbox) {
  ...
  projects.forEach((p) => {
    ...
    tableProject.setAttribute(
      "style",
      `width: ${(compT / totalT || 0) * 100}%`
    );
  });
}
...
if (taskCard) {
  upcomingTasks = tasks
    .sort((d1, d2) =>
      new Date(d1.dueDate).getTime() > new Date(d2.dueDate).getTime()
        ? 1
        : new Date(d1.dueDate).getTime() < new Date(d2.dueDate).getTime()
        ? -1
        : 0
...
upcomingTasks.forEach((t) => {
  ...
  dueDate.innerHTML = `<div class="cell-content">${new Date(
    t.dueDate
  ...
  t.assignees.forEach((a) => {
    ...
    aDiv.innerHTML = `<button type="button" class="btn btn-secondary rounded-circle" data-toggle="tooltip" data-placement="top" title=${
      a.email
    }>${a.email.charAt(0).toUpperCase()}</button>`;
