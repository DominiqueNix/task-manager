...
const handleTaskDelete = async (taskId) => {
  let projectId;
  projects.forEach((p) => {
    if (p.tasks.includes(taskId)) projectId = p._id;
  });
...
for (let i = 0; i < taskDeletebtns.length; i++) {
  ...
  currDeleteBtn.addEventListener("click", () => {
    handleTaskDelete(currId);
  });
}

//update a task
const updateTaskProjectData = document.getElementById("update-task-assignees");
const updateTaskProjects = JSON.parse(
  updateTaskProjectData.getAttribute("data-value")
);
...
for (let i = 0; i < updateTaskBtns.length; i++) {
  ...
  btn.addEventListener("click", () => {
    ...
    let updateTaskTitle = document.getElementById("update-task-title");
    let updateTaskDescription = document.getElementById(
    ...
    let projectDefalutforTask = document.getElementById(
      "project-with-update-task"
    );
    ...
    let updateTaskDefaultDate = new Date(
